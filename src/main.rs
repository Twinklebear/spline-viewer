...
use cgmath::{SquareMatrix, Transform, Vector2, Vector3};
use docopt::Docopt;
...
fn import_bspline(json: &serde_json::Value) -> BSpline<Point> {
    let degree = json["degree"]
        .as_u64()
        .expect("A curve degree must be specified") as usize;
    let points = json["points"]
        .as_array()
        .expect("A list of points must be specified")
        .iter()
        .map(|p| {
            Point::new(
                p["x"].as_f64().expect("Invalid x coord") as f32,
    ...
    if let Some(k) = json["knots"].as_array() {
        knots = k
            .iter()
            .map(|x| x.as_f64().expect("Invalid knot value") as f32)
            .collect();
    }
...
fn import_surf(json: &serde_json::Value) -> BSplineSurf<Point> {
    let u_data = json["u"]
        .as_object()
        .expect("Surface u component is required");
    let v_data = json["v"]
        .as_object()
        .expect("Surface v component is required");

    let degree_u = u_data["degree"]
        .as_u64()
        .expect("Surface u degree is required") as usize;
    let degree_v = v_data["degree"]
        .as_u64()
        .expect("Surface v degree is required") as usize;

    let knots_u = u_data["knots"]
        .as_array()
        .expect("Surface u knots are required")
        .iter()
        .map(|x| x.as_f64().expect("Invalid knot value") as f32)
        .collect();
    let knots_v = v_data["knots"]
        .as_array()
        .expect("Surface v knots are required")
        .iter()
        .map(|x| x.as_f64().expect("Invalid knot value") as f32)
        .collect();
    ...
    for r in json["mesh"]
        .as_array()
        .expect("Surface control mesh is required")
    {
        let points = r
            .as_array()
            .expect("A list of points must be specified")
            ...
            .map(|p| {
                Point::new(
                    p["x"].as_f64().expect("Invalid x coord") as f32,
                    p["y"].as_f64().expect("Invalid y coord") as f32,
                    p["z"].as_f64().expect("Invalid z coord") as f32,
                )
            })
            .collect();
        mesh.push(points);
...
fn import_surf_interpolation(json: &serde_json::Value) -> Vec<BSpline<Point>> {
    ...
    let degree_u = u_data["degree"]
        .as_u64()
        .expect("Surface u degree is required") as usize;
    let knots_u: Vec<f32> = u_data["knots"]
        .as_array()
        .expect("Surface u knots are required")
        .iter()
        .map(|x| x.as_f64().expect("Invalid knot value") as f32)
...
fn main() {
    ...
    for f in args.get_vec("<file>") {
        ...
        let json: serde_json::Value =
            serde_json::from_reader(reader).expect("Failed to read input file");
        let ty = json["type"]
            .as_str()
            .expect("A curve type must be specified");
        if ty == "bspline2d" {
            ...
            surface_interpolations.push(DisplaySurfInterpolation::new(
                import_surf_interpolation(&json),
                &display,
            ));
        } else {
    ...
    let mut ortho_proj = cgmath::ortho(
        width as f32 / -200.0,
        width as f32 / 200.0,
        height as f32 / -200.0,
        height as f32 / 200.0,
    ...
    let mut persp_proj =
        cgmath::perspective(cgmath::Deg(65.0), width as f32 / height as f32, 0.01, 100.0);
    ...
    'outer: loop {
        ...
        for e in display.poll_events() {
            match e {
                ...
                Event::MouseMoved(x, y)
                    if imgui.mouse_pressed.1 && !ui_interaction && !render_3d =>
                {
                    let delta = (
                        (x - imgui.mouse_pos.0) as f32 / (fbscale.0 * 100.0),
                ...
                Event::MouseMoved(x, y) if !ui_interaction && render_3d => {
                    if imgui.mouse_pressed.0 {
                        arcball_camera.rotate(
                            Vector2::new(imgui.mouse_pos.0 as f32, imgui.mouse_pos.1 as f32),
                            Vector2::new(x as f32, y as f32),
                        );
                    } else if imgui.mouse_pressed.1 {
                        let mouse_delta = Vector2::new(
                            (x - imgui.mouse_pos.0) as f32,
        ...
        if !ui_interaction {
            if render_3d {
                ...
                if imgui.mouse_pressed.0 && selected_curve < curves.len() as i32 {
                    let unproj = (ortho_proj * camera_2d.get_mat4())
                        .invert()
        ...
        ui_interaction = imgui.ui_interaction();

        let mut target = display.draw();
        target.clear_color(0.05, 0.05, 0.05, 1.0);

        let proj_view: [[f32; 4]; 4] = if !render_3d {
        ...
        ui.window(im_str!("Curve Control Panel"))
            ...
            .build(|| {
                ...
                for (i, c) in curves.iter_mut().enumerate() {
                    let idstack = ui.push_id(i as i32);
                    ui.separator();
                    ui.radio_button(im_str!("Select Curve"), &mut selected_curve, i as i32);
                    c.draw_ui(&ui);
                ...
                for (i, c) in curves3d.iter_mut().enumerate() {
                    let id = i + curves.len();
                    let idstack = ui.push_id(id as i32);
                    ui.separator();
                    ui.radio_button(im_str!("Select Curve"), &mut selected_curve, id as i32);
                    c.draw_ui(&ui);
                    if ui.small_button(im_str!("Remove Curve")) {
                        removing = Some(id);
                    }
                ...
                for (i, c) in surfaces.iter_mut().enumerate() {
                    ...
                    let idstack = ui.push_id(id as i32);
                    ui.separator();
                    ui.radio_button(im_str!("Select Surface"), &mut selected_curve, id as i32);
                    c.draw_ui(&ui);
                ...
                if let Some(i) = removing {
                    ...
                    if i >= curves.len() + curves3d.len() + surfaces.len() {
                        surface_interpolations
                            .remove(i - curves.len() - curves3d.len() - surfaces.len());
